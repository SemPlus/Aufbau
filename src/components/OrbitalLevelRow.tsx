...
export const OrbitalLevelRow: React.FC<OrbitalLevelRowProps> = ({ level, electronsInLevel, index }) => {
  ...
  return (
    <motion.div
      ...
      className="grid grid-cols-[60px_1fr_100px] items-center py-1.5 px-3 rounded-sm transition-colors hover:bg-yellow-500/5 group"
    >
      <div className="font-mono font-bold text-neutral-600 text-sm group-hover:text-neutral-400 transition-colors">
        {level.label}
      </div>
      ...
      <div className="text-right font-mono text-[10px] text-neutral-700">
        [{electronsInLevel} / {level.count * 2}]
      </div>
    </motion.div>
  );
};
